...
LANGUAGE SQL STABLE SET hnsw.ef_search = 500 AS $$
    ...
    chunk_ann AS (
        SELECT ch.cell_id, ch.page_from, ch.page_to,
    ...
    -- Best chunk per cell among the 400 ANN candidates above.
    chunk_best AS (
        SELECT DISTINCT ON (cell_id) cell_id, page_from, page_to, chunk_content, chunk_sem
        FROM chunk_ann ORDER BY cell_id, chunk_sem DESC
    ),
    ann AS (
    ...
    scored AS (
        SELECT c.id, c.content, c.summary, c.realm, c.signal, c.topic,
            ...
            -- i.e. chunk_sem is present and >= the cell vector's own similarity. At an exact tie
            -- the chunk counts as the provider (>=, not >): the value is identical either way, so
            -- reporting the more informative match location is the more useful reading.
            CASE WHEN cb.chunk_sem IS NOT NULL
                      AND cb.chunk_sem >= (CASE WHEN c.embedding IS NOT NULL AND query_embedding IS NOT NULL
                                                THEN (1 - ((c.embedding::vector(1024)) <=> query_embedding))::REAL
                                                ELSE 0::REAL END)
