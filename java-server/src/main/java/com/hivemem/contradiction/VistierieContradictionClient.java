...
/**
 * Dispatches a batch of candidate pairs to Vistierie's {@code contradiction-judge} agent.
 *
...
public class VistierieContradictionClient {
     ...
     * {@code VistierieContradictionClientTest} to inject a WireMock base URL directly), and
     * Spring's implicit single-constructor autowiring only kicks in when exactly one constructor
     * is declared. Without this annotation, Spring falls back to a no-arg instantiation strategy
     * and fails with "no default constructor found" the moment this bean is actually wired into an
     * application context (as {@link ContradictionSweep} now does).
     */
    @Autowired
    public VistierieContradictionClient(RestClient.Builder builder, QueenProperties props) {
        this(
                builder,
    ...
    /**
     * Create a run on Vistierie's contradiction-judge agent and return its run id.
     *
     ...
     *     was never created.
     */
    public String dispatch(UUID correlationId, List<PairPayload> pairs) {
        ...
        body.put("completion_webhook", callbackBaseUrl + "/vistierie/contradiction/done");
        body.put("completion_webhook_token", callbackToken);

        try {
            RunCreated created = client.post()
                    .uri("/agents/{name}/run", AgentDefinitions.CONTRADICTION_JUDGE_NAME)
