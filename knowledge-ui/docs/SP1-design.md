...
**Related HiveMem cells:** `ff28f80b` (Vision), `b2c9e9a3` (UI concept), `6eb85cf6` (3D-Pivot)
...
| Aspect | Decision |
|---|---|
| Top-level view | Realm-halos with cell-points inside |
| Zoom | Smooth continuous zoom + snap-focus on click |
| Hierarchy | Adaptive — single-child levels are transparently skipped |
| Sphere size | Hybrid metric with slider; default = cell count; alts = importance-weighted / content volume (char count) / popularity |
...
| Cell interaction | Scan mode (single-click slide-panel) + Reader mode (double-click fullscreen) |
...
stage (PIXI.Application root)
 └─ worldContainer        (zoom + pan transforms applied here)
     ...
     ├─ haloLayer         (Sprites with radial gradient per realm)
     ├─ spheresLayer      (Realm + Cell sprites)
...
Each realm has a **proceduraly-generated sprite texture** (256×256 offscreen canvas, then `PIXI.Texture.from(canvas)`): 3-stop radial gradient (glow/base/tint from `realmPalette.ts`, already in scaffold) + Perlin noise + slight displacement. Textures are cached per realm — one generation per realm, reused everywhere.

Cell points use a shared smaller texture tinted via `ColorMatrixFilter` to the realm colour.
...
- **Realm positions**: `d3-force` simulation with realm-to-realm tunnel counts as link strength. Runs once on load, settles within ~200 ticks, then frozen. Final positions written to localStorage per user; user can drag a realm and override. Drag triggers a 60-tick re-settle of that realm's neighbours, then freeze.
- **Cell positions within a realm halo**: deterministic Poisson-disk samples inside the halo radius, seeded by cell id. Gives even, non-overlapping distribution. New cells interpolate in (fade + grow over 400ms).

### Level of Detail (zoom-gated)

| Zoom range | What renders |
|---|---|
| < 0.3× | Realms only, flat circles, no labels |
| 0.3–1.0× | Realm halos + cell points, realm labels only |
| 1.0–3.0× | Cell labels on hover, edges full colour |
| > 3.0× | All labels, edge relation colours, click-ready hit boxes expand |

### Culling

Custom viewport-culling in the per-frame update: only sphere sprites whose world-AABB intersects the viewport get visible=true. Ensures smoothness at 10k+ cells even though SP1 will see ≤ 500 in practice.
...
| 🌟 Realms | Realms tree + tag/importance filters |
...
- `Enter` — open reader for focused cell
- `Space` — pan mode (hold)
- `+` / `-` — zoom in/out
- `0` — reset zoom to fit
...
### Scan mode (single click on cell sphere)
...
- **Right meta-panel** (collapsible): realm/signal/topic, importance, timestamps, tunnels shortcut, facts
...
- App loads in under 2 seconds against HiveMem production with 47 cells
...
- Playwright E2E smoke test passes: Login → Search → Open Cell → Enter Reader → Close Reader → Logout
...
5. **Canvas v1**: PixiJS application mounted in wrapper component, render hardcoded 2 realms + 5 cells to prove the pipeline, add bloom + outline filters
6. **Data integration**: connect `canvas` store to `ApiClient`, render real realms + cells from data
7. **Layout**: d3-force initial settle, freeze, Poisson-disk cell placement, localStorage persistence
...
10. **Scan mode**: `cell` store + right slide-panel + wire tunnels/attachments
...
- Writing cells from the UI (editor = SP4)
