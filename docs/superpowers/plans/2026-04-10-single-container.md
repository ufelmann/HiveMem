...
- [ ] **Step 1: Build image on the container**

```bash
ssh root@192.168.1.10 "pct exec 102 -- bash -c 'cd /root/hivemem && git pull && docker build -t hivemem .'"
```
...
```bash
ssh root@192.168.1.10 "pct exec 102 -- bash -c 'docker compose -f /root/hivemem/docker-compose.yml down -v'"
```

- [ ] **Step 3: Run single container**

```bash
ssh root@192.168.1.10 "pct exec 102 -- bash -c 'docker run -d --name hivemem -p 8421:8421 -v hivemem_data:/data --restart unless-stopped hivemem'"
```

- [ ] **Step 4: Wait for startup and check logs**

```bash
ssh root@192.168.1.10 "pct exec 102 -- docker logs -f hivemem"
```
...
```bash
ssh root@192.168.1.10 "pct exec 102 -- curl -s http://localhost:8421/mcp -H 'Content-Type: application/json' -d '{\"jsonrpc\":\"2.0\",\"id\":1,\"method\":\"tools/call\",\"params\":{\"name\":\"hivemem_health\",\"arguments\":{}}}'"
```
...
```bash
ssh root@192.168.1.10 "pct exec 102 -- docker exec hivemem hivemem-backup"
```
...
```bash
ssh root@192.168.1.10 "pct exec 102 -- docker exec hivemem psql -U hivemem -c 'SELECT count(*) FROM drawers'"
```
...
- [ ] **Step 2: Pull and rebuild on the container**

```bash
ssh root@192.168.1.10 "pct exec 102 -- bash -c 'cd /root/hivemem && git pull'"
ssh root@192.168.1.10 "pct exec 102 -- bash -c 'docker stop hivemem && docker rm hivemem || true'"
ssh root@192.168.1.10 "pct exec 102 -- bash -c 'cd /root/hivemem && docker build -t hivemem .'"
ssh root@192.168.1.10 "pct exec 102 -- bash -c 'docker run -d --name hivemem -p 8421:8421 -v hivemem_data:/data --restart unless-stopped hivemem'"
```
...
```bash
ssh root@192.168.1.10 "pct exec 102 -- docker image prune -f"
```
