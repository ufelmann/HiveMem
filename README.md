...
- **[Temporal Knowledge Graph](documentation/architecture.md#data-model)** — Facts with `valid_from`/`valid_until` and multi-hop graph traversal; query the graph as it stood at any point in time. **Subject canonicalization** (`kg_alias`) collapses fragmented subject strings ("Acme Inc." vs "Acme") into one canonical entity, retro-migrates existing facts, and is used by `kg_add`/`entity_overview` to resolve subjects on both the write and read path.
...
| [Tools](documentation/tools.md) | All 50 MCP tools (53 with `hivemem.contradiction.enabled=true`), the parallel REST attachment API, search signals, progressive summarization |
