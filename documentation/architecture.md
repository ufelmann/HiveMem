...
| **Write** | `add_cell`, `kg_add`, `kg_invalidate`, `revise_cell`, `revise_fact`, `reclassify`, `update_identity`, `update_blueprint`, `upload_attachment`, `save_search`, `delete_saved_search`, `manage_tags` | `agent` | Propose Change | Yes (for Agents) | Append-only knowledge capture; tag management; saved-search persistence. |
