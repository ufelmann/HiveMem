...
Fact extraction always runs before dedup gets a chance to see a cell — dedup needs the cell's embedding, which for long documents only exists once the summarizer (and with it, extraction) has run — so a discarded duplicate always carries its own extracted facts at the moment it is soft-deleted. Those facts are settled in the same transaction as the soft-delete and the `duplicate_of` tunnel, so a discard never leaves a live fact pointing at a dead cell: if the surviving original (or, if the original is itself no longer live, its live revision successor) already carries facts of its own, the duplicate's are invalidated; if it carries none, the duplicate's are handed over to it instead of being lost. If no live target can be resolved at all, the facts are left untouched and the cell is logged at `WARN` for manual review. See [Retro-settlement of orphaned facts](operations.md#retro-settlement-of-orphaned-facts) for the one-off backfill that applies the same rule to cells discarded before this existed.
