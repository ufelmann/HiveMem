...
50. `consumption_queue`: Scan ingest review queue: failed files (`state='failed'` regardless of remaining retry budget), batches whose degraded-page ratio crossed the flagging threshold (at least 2 degraded pages AND more than 2% of the batch's pages, see `ConsumptionQueueService`), the cumulative-since-startup reconciliation counters (`orphansRestaged`, `rowsWithoutFile`, `misplacedFailed` — see `ConsumptionRecoverySweep`), and per-state row counts. Optional param: `limit` (1–200, default 50, applied per section). Returns `{failedFiles: [...], degradedBatches: [...], reconciliation: {...}, stateCounts: {...}}`. When `hivemem.consumption.enabled=false` the underlying service is not registered and the tool returns an empty/`unavailable: true` shape instead of erroring — the tool itself is always present so the MCP surface (and this document's tool count) doesn't shrink when the pipeline is off.
51. `consumption_retry`: Re-stage one consumed scan file by its `sha256` so the pipeline ingests it again. Required param: `sha256`. Locates the physical file in `failed/`, `processing/` or `processed/` (the last covers a degraded batch that completed normally and needs a boundary re-run) and moves it back to the watch root — mirroring the recovery sweep, extended with the `processed/` case — WITHOUT touching the ledger row; the next poll re-hashes the file and resets the row itself. Returns `{sha256, restaged: true}` on success, or `{sha256, restaged: false, error: "..."}` if no ledger row matches the hash, no physical file exists in any of the three directories (the row is left untouched either way), or `hivemem.consumption.enabled=false` (`error: "consumption disabled"`). Content-based dedup (sha256 is the unique key) makes a re-run safe.
