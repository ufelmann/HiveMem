...
HiveMem exposes **50 MCP tools** by default, across search, knowledge graph, progressive summarization, agent fleet, references, attachments, saved searches, tag management, and admin — plus **3 more (53 total)** when `hivemem.contradiction.enabled=true` (`contradictions`, `resolve_contradiction`, `predicate_cardinality`; see below). Two of the 50 (`consumption_queue`, `consumption_retry`) cover the scan-ingest review queue; `consumption_queue` degrades to an empty/`unavailable` response rather than disappearing when `hivemem.consumption.enabled=false`. Large file uploads can also use the REST endpoint (`POST /api/attachments`) — see [Attachments](#attachments).

## Feature Overview

- **50 MCP tools** (53 with `hivemem.contradiction.enabled=true`) across search, knowledge graph, progressive summarization, agent fleet, references, attachments, saved searches, tag management, and admin
...
**Read (23, 24 with `hivemem.contradiction.enabled=true`):**
...
10. `pending_approvals`: List work awaiting review. No params. Each row returns `type` (`cell`|`fact`|`tunnel`), `id`, `title` (a short human label — `realm/topic` for a cell, `subject -> predicate -> object` for a fact, `<from label> -[relation]-> <to label>` for a tunnel; a cell label with no topic falls back to `realm/<first 8 characters of the cell id>`, with no realm it is the bare topic, and with neither it is the first 8 characters of the cell id alone), `description` (the cell summary or the tunnel's note — the proposing agent's rationale; null for facts), `realm`, `signal`, `from_cell`/`to_cell` (endpoint cell ids for tunnels, null otherwise), `created_by`, `created_at`. Pending tunnels are listed only when both endpoint cells are committed — a proposal hanging off a rejected or still-pending cell cannot be judged on its own merits and is excluded.
...
49. `archivist_log`: List recent moves and skips made by the inbox-archivist agent (see [Inbox Archivist](archivist.md)), newest first. Optional arg: `limit` (1–500, default 100). Returns `{entries:[{op_type,at,cell_id,...}]}`; `op_type` is `reclassify_cell` (a move — payload also carries `old_realm`/`old_topic`/`old_signal`, `new_realm`/`new_topic`/`new_signal`, `reason`) or `archivist_skip` (a skip — payload carries only `reason`).
50. `consumption_queue`: Scan ingest review queue: failed files (`state='failed'` regardless of remaining retry budget), batches flagged by either of two independent conditions (see `ConsumptionQueueService`/`ConsumptionFileRepository.findDegradedBatches`) — a degraded-page count at or above `min-degraded-pages` (default 1) that is also more than 2% of the batch's pages, OR a blank-page ratio (`blank_pages`/`total_pages`) above `blank-ratio-alert` (default 0.60) — each returned row carrying `totalPages`, `degradedPages` and `blankPages` (nullable on rows recorded before the `blank_pages` column existed), rows that stalled without failing (still `staged` or `processing` past `recovery-stale-threshold`, each with `sha256`, `filename`, `state`, `updatedAt`, `ageSeconds`), the cumulative-since-startup reconciliation counters (`orphansRestaged`, `rowsWithoutFile`, `misplacedFailed` — see `ConsumptionRecoverySweep`), and per-state row counts. `failedFiles` is ordered newest-first (the retry sweep's own oldest-first ordering is unaffected) and is not filtered by remaining retry budget. `stateCounts` is an all-time census of the ledger, not a queue depth: it counts every row, including the monotonically growing `done` pile. Optional param: `limit` (1–200, default 50, applied per section). Returns `{failedFiles: [...], degradedBatches: [...], stalledRows: [...], reconciliation: {...}, stateCounts: {...}}`. When `hivemem.consumption.enabled=false` the underlying service is not registered and the tool returns an empty/`unavailable: true` shape instead of erroring — the tool itself is always present so the MCP surface (and this document's tool count) doesn't shrink when the pipeline is off.
51. `consumption_retry`: Re-stage one consumed scan file by its `sha256` so the pipeline ingests it again. Required param: `sha256`. Locates the physical file in `failed/`, `processing/` or `processed/` (the last covers a degraded batch that completed normally and needs a boundary re-run) and moves it back to the watch root — mirroring the recovery sweep, extended with the `processed/` case — WITHOUT touching the ledger row. Returns `{sha256, restaged: true}` on success, or `{sha256, restaged: false, error: "..."}` in four cases: no ledger row matches the hash (`"unknown sha256"`), no physical file exists in any of the three directories (`"no physical file for '<name>' in failed/, processing/ or processed/"`), the move itself failed (`"move failed: ..."`), or `hivemem.consumption.enabled=false` (`"consumption disabled"`). The row is left untouched in every failure case. On success the row is left untouched too: the next poll re-hashes the file and calls `stage()`, which resets `state` to `staged` and refreshes `filename` — it does NOT clear `attempts`, `last_error`, `total_pages`, `degraded_pages` or `blank_pages`, so a retried row keeps its history until the new pass overwrites those columns. Content-based dedup (sha256 is the unique key) makes a re-run safe.
52. `resolve_contradiction` (only registered when `hivemem.contradiction.enabled=true`): Resolve a pending or deferred contradiction pair: keep one fact and invalidate the other, dismiss both as legitimate, or requeue for re-judging. Required params: `id` (contradiction pair UUID), `keep` (`fact_a`|`fact_b`|`both`|`requeue`), `reason` (human-readable, audit only, not persisted). **`keep=fact_a` or `keep=fact_b` invalidates the other fact through the same op-logged `kg_invalidate` path used everywhere else — this is irreversible**: the losing fact's `valid_until` is set and it stops being an active fact.
53. `predicate_cardinality` (only registered when `hivemem.contradiction.enabled=true`): List predicate cardinality verdicts (Stage-A judge output plus any human overrides), or — with `set` — record a human override. List mode takes an optional `predicate` filter. Setting requires `predicate`, `set` (`single_valued`|`multi_valued`), and `reason`. A human override outranks every future judge verdict for that predicate permanently; setting `multi_valued` also supersedes the predicate's open contradiction pairs (multi-valued predicates cannot contradict by design).

Both `resolve_contradiction` and `predicate_cardinality` are deliberately registered in `ADMIN_TOOLS`, not `WRITE_TOOLS`. In this codebase `AGENT_TOOLS == WRITER_TOOLS`, so a `WRITE` classification would let any agent-role token invalidate a committed fact through `resolve_contradiction` — the very role whose own writes are forced to `pending` precisely so a human has to approve them. Keeping both tools admin-only is what makes "a human chose the winner" actually true.
