...
51. `consumption_retry`: Re-stage one consumed scan file by its `sha256` so the pipeline ingests it again. Required param: `sha256`. Returns `{sha256, restaged: true}`, or `{sha256, restaged: false, error: "unknown sha256"}` if no ledger row matches. Content-based dedup (sha256 is the unique key) makes a re-run safe.
