...
51. `consumption_retry`: Re-stage one consumed scan file by its `sha256` so the pipeline ingests it again. Required param: `sha256`. Locates the physical file in `failed/` or `processing/` and moves it back to the watch root — mirroring the recovery sweep — WITHOUT touching the ledger row; the next poll re-hashes the file and resets the row itself. Returns `{sha256, restaged: true}` on success, or `{sha256, restaged: false, error: "..."}` if no ledger row matches the hash, no physical file exists in either directory (the row is left untouched either way), or `hivemem.consumption.enabled=false` (`error: "consumption disabled"`). Content-based dedup (sha256 is the unique key) makes a re-run safe.
